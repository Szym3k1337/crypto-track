...
import CryptoModel3D from "./CryptoModel3D.tsx";
import CryptoMappedModel from "./CryptoMappedModel.tsx";
import {Canvas} from "@react-three/fiber";
import {OrbitControls} from "@react-three/drei";
import CryptoChart from "./CryptoChart.tsx";
interface CryptoDetail {
    type:"model" | "procedural";
    path:string;
    scale?: number;
    name:string;
    description:string;
...
function CryptoDetail () {
    ...
    const CRYPTO_CONFIG: Record<string, CryptoDetail> = {
        bitcoin: {
            type: 'model',
            path: '/bitcoin.glb',
            scale: 50,
    ...
    const config = id ? CRYPTO_CONFIG[id]: undefined;
    if(!config) {
        return(
            <main className="min-h-screen bg-slate-950 text-slate-100 flex flex-col items-center justify-center p-6 text-center">
                <h1 className="text-2xl font-bold mb-4">Nie znaleziono takiej kryptowaluty</h1>
                <Link to="/" className="text-sky-400 hover:underline">← Powrót do Dashboardu</Link>
            </main>
        )
    }
    ...
    return (
        <main className="min-h-screen bg-slate-950 text-slate-100 flex flex-col items-center justify-center p-6 font-sans gap-4">
            ...
            <Canvas>
                ...
                {config.type === 'model' ? (
                    <CryptoModel3D modelPath={config.path} scale={config.scale ?? 1} />
                ) : (
