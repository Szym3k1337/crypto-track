...
import ChooseCryptoModel from "./ChooseCryptoModel.tsx";
...
function CryptoPrice(){
    const { data, loading, error, handleFetchData } = useFetch<ServerResponse>("https://api.coingecko.com/api/v3/simple/price?ids=bitcoin,ethereum,tether,binancecoin,ripple,solana&vs_currencies=usd");
    ...
    return (
        <main id="Screen" className="min-h-screen bg-slate-950 text-slate-100 flex flex-col items-center justify-center p-6 font-sans">
            ...
            <section id="cards-grid" className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 max-w-5xl w-full mb-8">
                ...
                <CryptoCard name="bitcoin" shortName="BTC" price={data?.bitcoin?.usd ?? 0} colorClass="text-amber-500" id='bitcoin' />
                <CryptoCard name="ethereum" shortName="ETH" price={data?.ethereum?.usd ?? 0} colorClass="text-indigo-400" id='ethereum' />
                <CryptoCard name="tether" shortName="USDT" price={data?.tether?.usd ?? 0} colorClass="text-emerald-400" id='usdt' />
                <CryptoCard name="binance coin" shortName="BNB" price={data?.binancecoin?.usd ?? 0} colorClass="text-yellow-500" id='bnb' />
                <CryptoCard name="ripple" shortName="XRP" price={data?.ripple?.usd ?? 0} colorClass="text-sky-400" id='xrp'/>
                <CryptoCard name="solana" shortName="SOL" price={data?.solana?.usd ?? 0} colorClass="text-purple-400" id='sol' />
